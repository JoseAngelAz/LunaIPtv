package com.lunaiptv.features.settings.data
...
import com.lunaiptv.features.home.HomeConfig
...
class SettingsRepository(private val context: Context) {
    ...
    val animationLevel: Flow<com.lunaiptv.ui.theme.AnimationLevel> = context.dataStore.data.map { prefs ->
        prefs[Keys.ANIMATION_LEVEL]?.let { runCatching { com.lunaiptv.ui.theme.AnimationLevel.valueOf(it) }.getOrNull() }
            ?: com.lunaiptv.ui.theme.AnimationLevel.FULL
    }

    suspend fun setAnimationLevel(level: com.lunaiptv.ui.theme.AnimationLevel) {
    ...
    val metadataMode: Flow<com.lunaiptv.core.metadata.MetadataMode> = context.dataStore.data.map { p ->
        parseMetadataMode(p)
    }

    private fun parseMetadataMode(p: Preferences): com.lunaiptv.core.metadata.MetadataMode {
        p[Keys.METADATA_MODE]?.let { raw ->
            runCatching { com.lunaiptv.core.metadata.MetadataMode.valueOf(raw) }.getOrNull()?.let { return it }
        }
        // No explicit mode yet — derive from the legacy boolean toggle.
        return if (p[Keys.METADATA_ENABLED] == false) com.lunaiptv.core.metadata.MetadataMode.PROVIDER
        else com.lunaiptv.core.metadata.MetadataMode.PROVIDER_PLUS_TMDB
    }

    suspend fun setMetadataMode(mode: com.lunaiptv.core.metadata.MetadataMode) {
    ...
    val metadataConfigFlow: Flow<com.lunaiptv.core.metadata.MetadataConfig> = context.dataStore.data.map { p ->
        com.lunaiptv.core.metadata.MetadataConfig(
    ...
    suspend fun metadataConfig(): com.lunaiptv.core.metadata.MetadataConfig = metadataConfigFlow.first()
    ...
    /** Default zoom/aspect mode applied when playback starts (a [com.lunaiptv.player.ZoomMode] name). */
    ...
    val proxyConfig: Flow<com.lunaiptv.core.network.ProxyConfig> = context.dataStore.data.map { p ->
        com.lunaiptv.core.network.ProxyConfig(
