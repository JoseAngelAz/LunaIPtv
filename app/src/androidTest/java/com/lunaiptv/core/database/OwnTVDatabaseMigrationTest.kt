...
class LunaIPtvDatabaseMigrationTest {
    ...
    private fun bootstrapVersion2Database() {
        ...
        try {
            executeSchemaQueries(db, "com.lunaiptv.core.database.OwnTVDatabase/2.json")
            seedVersion2Data(db)
            db.version = 2
        } finally {
            db.close()
    ...
    private fun bootstrapVersion7Database() {
        val db = context.openOrCreateDatabase(DB_NAME, Context.MODE_PRIVATE, null)
        try {
            executeSchemaQueries(db, "com.lunaiptv.core.database.OwnTVDatabase/7.json")
            seedVersion7Data(db)
            db.version = 7
        } finally {
            db.close()
    ...
    private fun bootstrapVersion3Database() {
        val db = context.openOrCreateDatabase(DB_NAME, Context.MODE_PRIVATE, null)
        try {
            executeSchemaQueries(db, "com.lunaiptv.core.database.OwnTVDatabase/3.json")
            seedVersion3Data(db)
            db.version = 3
        } finally {
            db.close()
